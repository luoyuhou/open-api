...
export class WxService {
  ...
  public async recommendStoresForMiniApp(pageNum: number, pageSize: number) {
    ...
    >`SELECT COUNT(*) AS total
      FROM storehouse.store AS s
      INNER JOIN storehouse.store_rating AS r ON s.store_id = r.store_id
  ...
  public async listFavoriteStores(
    ...
    const totalRows = await this.prisma.$queryRaw<
      { total: bigint }[]
    >`SELECT COUNT(*) AS total
      FROM storehouse.user_favorite_store AS f
    ...
    const rows = await this.prisma.$queryRaw<any[]>`
      SELECT
        ...
        r.rating
      FROM storehouse.user_favorite_store AS f
      LEFT JOIN storehouse.store AS s ON f.store_id = s.store_id
      LEFT JOIN storehouse.province AS B1 ON s.province = B1.code
      LEFT JOIN storehouse.province AS B2 ON s.city = B2.code
      LEFT JOIN storehouse.province AS B3 ON s.area = B3.code AND B3.town = 0
      LEFT JOIN storehouse.province AS B4 ON s.area = B4.code AND s.town = B4.town
      LEFT JOIN storehouse.store_rating AS r ON s.store_id = r.store_id
      WHERE f.user_id = ${userId}
  ...
  public async listStoreBrowseHistory(
    ...
    const totalRows = await this.prisma.$queryRaw<
      { total: bigint }[]
    >`SELECT COUNT(*) AS total
      FROM storehouse.user_store_browse_history AS h
    ...
    const rows = await this.prisma.$queryRaw<any[]>`
      SELECT
        ...
        r.rating
      FROM storehouse.user_store_browse_history AS h
      LEFT JOIN storehouse.store AS s ON h.store_id = s.store_id
      LEFT JOIN storehouse.province AS B1 ON s.province = B1.code
      LEFT JOIN storehouse.province AS B2 ON s.city = B2.code
      LEFT JOIN storehouse.province AS B3 ON s.area = B3.code AND B3.town = 0
      LEFT JOIN storehouse.province AS B4 ON s.area = B4.code AND s.town = B4.town
